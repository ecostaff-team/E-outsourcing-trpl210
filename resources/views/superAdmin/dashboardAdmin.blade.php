    ...
    <body class="bg-gray-100" style="font-family: 'Poppins', sans-serif;">
        ...
        <div class="p-4 md:p-8 lg:p-10">
            ...
            <div class="bg-white px-4 md:px-10 border-gray-200 border-b border-t flex gap-6 text-sm overflow-x-auto">
                <button onclick="switchTab('admin')" id="tabAdmin"
                    class="py-3 border-b-2 border-green-600 text-green-700 font-semibold">
                    Admin Outsourcing
                </button>
            ...
            <div class="bg-white p-8 rounded-lg shadow-lg mt-6">
                <div class="flex flex-col md:flex-row md:justify-between gap-3 mb-4">
                    ...
                    <button class="bg-green-600 shadow-lg text-white px-4 py-2 rounded-lg text-sm">+ Tambah
                        Akun</button>
                </div>
            ...
            <div class="bg-white w-full max-w-xl rounded-xl overflow-y-auto shadow-lg">
                ...
                <div class="p-10 grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    <div class="md:col-span-2">
                        <label class="font-bold">Nama <span class="text-red-500">*</span></label>
                        <input type="text" placeholder="Contoh: Rizky Darmawan"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="email" placeholder="email@domain.co.id"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 mt-1">
                    </div>
                    <div class="md:col-span-2">
                        <label class="font-bold">NIP <span class="text-red-500">*</span></label>
                        <input type="text" placeholder="Contoh: 12345678"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 mt-1">
                    </div>
                    <div class="md:col-span-2">
                ...
                <div class="flex flex-col md:flex-row justify-end gap-3 px-5 py-4 border-t border-gray-200">
                    <button onclick="closeModal()"
                        class="px-4 py-2 border border-gray-300 rounded-lg w-full md:w-auto hover:bg-w-800 hover:shadow-2xl transform hover:-translate-y-0.5 transition-all duration-200">Batal</button>
                    <button
                        class="px-4 py-2 bg-green-700 text-white rounded-lg w-full md:w-auto shadow-xl hover:bg-green-800 hover:shadow-2xl transform hover:-translate-y-0.5 transition-all duration-200">Simpan</button>
                </div>
            </div>
        </div>
        ...
        <script>
            const adminList = [{
                    nama: "Rizky Darmawan",
                    email: "rizky.darmawan@vendor.co.id",
                    nip: "12345678",
                    status: "Aktif",
                    telp: "085762360846",
                    tanggal: "10 Apr 2026"
            ...
            function switchTab(tab) {
                ...
                if (tab === 'admin') renderTable(adminList);
                if (tab === 'hr') renderTable(hrList);
                if (tab === 'kepala') renderTable(kepalaList);
                ...
                if (tab === 'admin') {
                    document.getElementById('tabAdmin').className =
                        "py-3 border-b-2 border-green-600 text-green-700 font-semibold";
                }
                if (tab === 'hr') {
                    document.getElementById('tabHR').className =
                        "py-3 border-b-2 border-green-600 text-green-700 font-semibold";
                }
                if (tab === 'kepala') {
                    document.getElementById('tabKepala').className =
                        "py-3 border-b-2 border-green-600 text-green-700 font-semibold";
                }
            }
        </script>
        ...
        <script>
            ...
            function renderTable(data) {
                ...
                table.innerHTML = paginated.map((item, index) => `
        ...
        `).join('');
            ...
            function renderPagination() {
                ...
                const container = document.querySelector('.flex.justify-end.mt-4');

                let buttons = `
            <button onclick="changePage(${currentPage - 1})" class="px-3 py-1 border rounded">Previous</button>
        `;

                for (let i = 1; i <= totalPages; i++) {
                    buttons += `
                <button onclick="changePage(${i})" class="px-3 py-1 ${i === currentPage ? 'bg-green-600 text-white' : 'border'} rounded">${i}</button>
            `;
                }

                buttons += `
            <button onclick="changePage(${currentPage + 1})" class="px-3 py-1 border rounded">Next</button>
        `;
        ...
        <div id="modalEdit" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-50 px-4">
            <div class="bg-white w-full max-w-xl rounded-xl overflow-y-auto shadow-lg">
                ...
                <div class="p-10 grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    <div class="md:col-span-2"> <label class="font-bold">Nama <span
                                class="text-red-500">*</span></label>
                        <input id="editNama" type="text" placeholder="Contoh: Rizky Darmawan"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 mt-1">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <label class="font-bold">No Telepon <span class="text-red-500">*</span></label>
                        <input type="text" id="editNomor" placeholder="Contoh: 081234567890"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 mt-1">
                    </div>
        ...
        <script>
            function editUser(name) {
                const user = currentData.find(u => u.nama === name);

                document.getElementById('editNama').value = user.nama;
                document.getElementById('editEmail').value = user.email;
                document.getElementById('editNomor').value = user.telp;
